...
### 8.c — Enable auto-merge
\`\`\`bash
gh pr merge --auto --squash "$PR_URL"
AUTO_EXIT=$?
\`\`\`
- If AUTO_EXIT=0 → write .done with status=done, phase=complete, prUrl=\$PR_URL, autoMerge=true:
\`\`\`bash
cat > ${logDir}/${taskId}.done <<'DONE_AUTO'
{"taskId":"${taskId}","status":"done","phase":"complete","prUrl":"<PR_URL>","autoMerge":true,"branch":"${branchName}","summary":"PR open; auto-merge queued — will merge when checks pass","lastUpdate":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","finishedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
DONE_AUTO
\`\`\`
- If AUTO_EXIT≠0, inspect stderr:
  - If stderr contains "auto-merge is not enabled" OR "auto merge is disabled":
    Write .done with autoMerge=false and include autoMergeError; add note:
    "Auto-merge unavailable — enable in repo Settings → General → Pull Requests → Allow auto-merge,
     then manually run: gh pr merge --auto --squash <pr-number>"
\`\`\`bash
cat > ${logDir}/${taskId}.done <<'DONE_NO_AUTO'
{"taskId":"${taskId}","status":"done","phase":"complete","prUrl":"<PR_URL>","autoMerge":false,"autoMergeError":"<stderr>","branch":"${branchName}","summary":"PR open; auto-merge unavailable — enable in repo settings then run: gh pr merge --auto --squash <pr-number>","lastUpdate":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","finishedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
DONE_NO_AUTO
\`\`\`
  - Other errors: log but treat PR creation as success (PR is open, ready for manual merge). Write .done with autoMerge=false, autoMergeError=<error>.`
