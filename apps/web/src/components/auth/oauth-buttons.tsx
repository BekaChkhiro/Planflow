...
import { useGitHubOAuth, useGoogleOAuth } from '@/hooks/use-oauth-auth'
...
export function OAuthButtons({
  ...
  const { startGitHubLogin, isLoading: isGitHubLoading, error: gitHubError } = useGitHubOAuth()
  const { startGoogleLogin, isLoading: isGoogleLoading, error: googleError } = useGoogleOAuth()
  ...
  const handleGitHubClick = async () => {
    ...
    try {
      await startGitHubLogin(redirectUrl)
    } catch {
  ...
  const handleGoogleClick = async () => {
    ...
    try {
      await startGoogleLogin(redirectUrl)
    } catch {
      setLoadingProvider(null)
    }
  }
