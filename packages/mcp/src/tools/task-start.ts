...
const TaskStartInputSchema = z.object({
  ...
  mergeStrategy: z
    .enum(['pr', 'auto-merge', 'merge-master', 'none'])
    .optional()
    .default('pr')
    .describe(
      'How the autonomous agent finishes the task:\n' +
        '  • pr (default): opens a pull request; user merges manually after review.\n' +
        '  • auto-merge: opens a PR AND enables GitHub auto-merge (--squash) so the\n' +
        '                PR merges automatically when all status checks pass.\n' +
        '                Best for branch-protected repos with full CI coverage.\n' +
        '                Requires: repo Settings → General → Pull Requests → "Allow auto-merge" enabled,\n' +
        '                and branch protection with required status checks.\n' +
        '  • merge-master: merges to the main branch directly and pushes (destructive,\n' +
        '                  only works when master is not branch-protected).\n' +
        '  • none: leaves the branch unmerged and unpushed (local only).\n' +
        'Only used when autoExecute=true.'
    ),
  model: z
...
interface DispatchOpts {
  ...
  mergeStrategy: 'pr' | 'auto-merge' | 'merge-master' | 'none'
...
CHECKPOINT
\`\`\`
- Open a pull request (no Co-Authored-By in the body). Derive the default
  branch so the PR targets \`main\` / \`master\` correctly even after rebases
  that may have changed local upstream tracking:
\`\`\`bash
DEFAULT_BRANCH=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's|^origin/||')
DEFAULT_BRANCH=\${DEFAULT_BRANCH:-main}
gh pr create --base "$DEFAULT_BRANCH" --head ${branchName} --title "feat(<scope>): ${taskId.toLowerCase()} — ${taskName.toLowerCase()}" --fill
\`\`\`
  Capture the PR URL from gh output. If gh is not available, log a clear message and skip.`
      : mergeStrategy === 'auto-merge'
        ? `## 8. Push branch, open PR, and enable auto-merge
...
### 8.b — Open PR
Discover the repo's actual default branch — many projects use \`main\` (not \`master\`),
and after a stale-base rebase the local upstream may point at another task branch.
Hardcoding fails silently: \`gh pr create --base master\` against a \`main\` repo
falls back to \`--fill\`'s auto-detection, which can pick the wrong base branch.

\`\`\`bash
DEFAULT_BRANCH=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's|^origin/||')
if [ -z "$DEFAULT_BRANCH" ]; then
  # Symref not set locally — ask remote directly.
  DEFAULT_BRANCH=$(git ls-remote --symref origin HEAD 2>/dev/null | sed -n 's|^ref: refs/heads/\\(.*\\)\\sHEAD|\\1|p')
fi
DEFAULT_BRANCH=\${DEFAULT_BRANCH:-main}
echo "Default branch resolved to: $DEFAULT_BRANCH"

PR_URL=$(gh pr create \\
  --base "$DEFAULT_BRANCH" \\
  --head ${branchName} \\
  --title "feat(<scope>): ${taskId.toLowerCase()} — ${taskName.toLowerCase()}" \\
  --body "Auto-generated by autoExecute.")
\`\`\`
Capture the PR URL and number from the output.

**Verify** the PR's base is actually \`$DEFAULT_BRANCH\` and not another task branch
(can happen if the worktree was rebased onto a sibling task's branch):
\`\`\`bash
PR_BASE=$(gh pr view "$PR_URL" --json baseRefName --jq .baseRefName)
if [ "$PR_BASE" != "$DEFAULT_BRANCH" ]; then
  echo "ERROR: PR opened against '$PR_BASE' instead of '$DEFAULT_BRANCH'. Fixing..."
  gh pr edit "$PR_URL" --base "$DEFAULT_BRANCH"
fi
\`\`\`

### 8.c — Enable auto-merge (best-effort)
\`\`\`bash
AUTO_MERGE_OK=true
gh pr merge --auto --squash "$PR_URL" 2>/tmp/automerge_stderr_${taskId}
AUTO_EXIT=$?
if [ $AUTO_EXIT -ne 0 ]; then
  AUTO_MERGE_OK=false
  AUTO_MERGE_ERROR=$(cat /tmp/automerge_stderr_${taskId} 2>/dev/null | tr '\\n' ' ' | head -c 300)
  echo "Auto-merge enable failed (will still poll/recover): $AUTO_MERGE_ERROR"
fi
\`\`\`

**Important:** even if auto-merge enable failed (repo doesn't have "Allow auto-merge" turned on,
or there's a protection issue), STILL proceed to step 8.d to poll and recover. If the PR is
DIRTY/CONFLICTING you should fix it. If it eventually becomes mergeable, the user can merge
it manually — but the task does NOT get marked done unless an actual merge lands in master.

### 8.d — Wait for actual merge with self-healing recovery
Auto-merge is queued at this point, but the PR only merges when:
  • Required CI checks pass
  • Branch is up-to-date with master (if "Require up-to-date" is on)
  • Required reviews approve (if any)

The poll loop below blocks until ONE of:
  • PR merges (success path)
  • A terminal failure that can't be recovered (closed PR, unresolvable conflict)
  • Recovery is needed — agent takes over, fixes, re-pushes, loops back

Up to **3 recovery attempts** total before giving up. Each attempt can be
DIRTY (merge conflict) or CI_FAILED — both flow into the recovery block below.
...
MAX_RECOVERIES=3
START_TIME=$(date +%s)
OVERALL_TIMEOUT=3600   # 60 min absolute cap across all recoveries
INTERVAL=30

# ── Main loop: poll → maybe recover → poll again ──
while true; do
  NOW=$(date +%s)
  ELAPSED=$((NOW - START_TIME))
  if [ $ELAPSED -gt $OVERALL_TIMEOUT ]; then
    PR_FINAL_STATE="TIMEOUT_AFTER_\${OVERALL_TIMEOUT}S"
    break
  fi

  # Re-enable auto-merge after every recovery push. It gets cleared when
  # branch HEAD changes on the PR, so we always re-arm before polling.
  # Failure is OK (repo may not have "Allow auto-merge" enabled); we still
  # poll and recover. If we end up MERGEABLE without auto-merge, we mark as
  # AWAITING_MANUAL_MERGE rather than spinning forever.
  AUTO_MERGE_REARMED=true
  gh pr merge --auto --squash "$PR_URL" 2>/dev/null || AUTO_MERGE_REARMED=false

  # Inner poll: detect terminal state or break-for-recovery
  POLL_OUTCOME=""
  MERGEABLE_GREEN_COUNT=0   # how many consecutive cycles we've seen "clean but unmerged"
  while [ $((($(date +%s)) - START_TIME)) -lt $OVERALL_TIMEOUT ]; do
    PR_JSON=$(gh pr view "$PR_URL" --json state,mergeStateStatus,mergedAt,mergeCommit,statusCheckRollup 2>/dev/null)
    STATE=$(echo "$PR_JSON" | grep -o '"state":"[^"]*"' | head -1 | cut -d'"' -f4)
    MERGE_STATUS=$(echo "$PR_JSON" | grep -o '"mergeStateStatus":"[^"]*"' | head -1 | cut -d'"' -f4)
    CHECKS_FAILED=$(echo "$PR_JSON" | grep -o '"conclusion":"FAILURE"' | wc -l | tr -d ' ')

    if [ "$STATE" = "MERGED" ]; then
      PR_MERGED=true
      PR_FINAL_STATE="MERGED"
      PR_MERGE_COMMIT=$(echo "$PR_JSON" | grep -o '"oid":"[^"]*"' | head -1 | cut -d'"' -f4)
      POLL_OUTCOME="MERGED"
      break
    fi

    if [ "$STATE" = "CLOSED" ]; then
      PR_FINAL_STATE="CLOSED_WITHOUT_MERGE"
      POLL_OUTCOME="CLOSED"
      break
    fi

    if [ "$MERGE_STATUS" = "BEHIND" ]; then
      # Cheap recovery, no claude reasoning needed — GitHub does the merge commit.
      gh pr update-branch "$PR_URL" 2>&1 | head -3 || true
    ...
    if [ "$CHECKS_FAILED" -gt 0 ]; then
      POLL_OUTCOME="CI_FAILED"
      # Capture the failing run URL for the agent to inspect
      FAILED_RUN_URL=$(echo "$PR_JSON" | grep -B1 '"conclusion":"FAILURE"' | grep '"detailsUrl"' | head -1 | grep -o 'https://[^"]*')
      break
    fi

    # PR is clean (MERGEABLE / CLEAN / UNSTABLE-no-failures) but not merged.
    # If auto-merge couldn't be enabled, GitHub won't merge it for us —
    # we'd spin forever. Detect after 3 consecutive clean cycles (~90s)
    # and exit so the agent doesn't burn the full hour for nothing.
    if [ "$AUTO_MERGE_REARMED" = "false" ]; then
      MERGEABLE_GREEN_COUNT=$((MERGEABLE_GREEN_COUNT + 1))
      if [ $MERGEABLE_GREEN_COUNT -ge 3 ]; then
        POLL_OUTCOME="AWAITING_MANUAL_MERGE"
        PR_FINAL_STATE="AWAITING_MANUAL_MERGE"
        break
      fi
    fi
  ...
  # ── Decide what to do based on POLL_OUTCOME ──
  if [ "$POLL_OUTCOME" = "MERGED" ] || [ "$POLL_OUTCOME" = "CLOSED" ] || [ "$POLL_OUTCOME" = "AWAITING_MANUAL_MERGE" ]; then
    break  # terminal, exit main loop
  fi
  ...
  # We hit DIRTY or CI_FAILED — need claude-driven recovery
  if [ $RECOVERIES -ge $MAX_RECOVERIES ]; then
    PR_FINAL_STATE="MAX_RECOVERIES_EXCEEDED_\${POLL_OUTCOME}"
    break
  fi

  RECOVERIES=$((RECOVERIES + 1))
  echo "Recovery attempt $RECOVERIES/$MAX_RECOVERIES — outcome was $POLL_OUTCOME"
  echo "RECOVERY_NEEDED=$POLL_OUTCOME"
  if [ -n "$FAILED_RUN_URL" ]; then
    echo "FAILED_RUN_URL=$FAILED_RUN_URL"
  fi
  break  # exit this iteration so claude can read the result and do step 8.d.r
done
\`\`\`

### 8.d.r — Recovery block (only runs if RECOVERY_NEEDED is set)
**Skip this step entirely if the previous bash output did NOT print \`RECOVERY_NEEDED=\`.**

You (claude, the agent) are taking over here. Two paths:

**Path A — RECOVERY_NEEDED=DIRTY** (merge conflict)
1. \`git fetch origin master\`
2. \`git rebase origin/master\` from inside the worktree (\`${agentCwd}\`)
3. If conflicts appear:
   - For each conflicted file: open it, understand both sides, write a sensible merged version.
   - Most conflicts on this project are imports / formatting / unrelated lines — resolvable.
   - If a conflict requires deep design decisions you can't make, STOP, set \`PR_FINAL_STATE=DIRTY_UNRESOLVABLE\`, jump to 8.e.
   - Otherwise: \`git add <file>\`, \`git rebase --continue\`.
4. After successful rebase: \`git push --force-with-lease\`
5. Re-enter step 8.d (set \`RECOVERIES\` to keep its current value — the bash loop handles the count).

**Path B — RECOVERY_NEEDED=CI_FAILED** (CI run failed)
1. From \`FAILED_RUN_URL\` extract the run ID (last numeric segment).
2. Inspect the failed logs:
\`\`\`bash
RUN_ID=$(echo "$FAILED_RUN_URL" | grep -oE '[0-9]+$')
gh run view "$RUN_ID" --log-failed 2>/dev/null | tail -200
\`\`\`
3. Read the failing logs carefully. Identify:
   - Lint / type-check errors → fix the obvious issue (missing import, type mismatch, unused var).
   - Test failures → read the test assertion + the file under test, fix the bug or update the test if it's stale.
   - Build errors → usually missing files or imports.
4. Apply the smallest plausible fix. Run \`pnpm typecheck\` and \`pnpm test\` locally first if available.
5. \`git add <changed>\`, \`git commit -m "fix(${taskId}): address CI feedback — <one-line>"\`, \`git push\`.
6. Re-enter step 8.d.

**Limits:**
- Max 3 recovery attempts total (DIRTY + CI_FAILED combined).
- Overall timeout: 60 min. After that, mark as TIMEOUT and stop.
- If you can't figure out a fix in one read pass, DON'T flail — set \`PR_FINAL_STATE=CI_FAILED_UNFIXABLE\` and move to 8.e. A confused fix is worse than honest failure.

After step 8.d.r completes (successful push or giving up), JUMP BACK to step 8.d.
The bash loop's \`RECOVERIES\` counter persists; you'll go through one more poll cycle.

### 8.e — Write final .done marker

When 8.d's main loop exits, use the final variables to write the marker:

If \`PR_MERGED=true\`:
\`\`\`bash
cat > ${logDir}/${taskId}.done <<DONE_MERGED
{"taskId":"${taskId}","status":"done","phase":"merged","prUrl":"$PR_URL","autoMerge":true,"merged":true,"mergeCommit":"$PR_MERGE_COMMIT","recoveries":$RECOVERIES,"branch":"${branchName}","summary":"PR merged into master ($PR_MERGE_COMMIT) after $RECOVERIES recovery attempts.","lastUpdate":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","finishedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
DONE_MERGED
\`\`\`

If NOT merged (\`PR_MERGED=false\`):
\`\`\`bash
cat > ${logDir}/${taskId}.done <<DONE_NOT_MERGED
{"taskId":"${taskId}","status":"in-progress","phase":"awaiting-merge","prUrl":"$PR_URL","autoMerge":true,"merged":false,"finalState":"$PR_FINAL_STATE","recoveries":$RECOVERIES,"branch":"${branchName}","summary":"Self-heal exhausted ($PR_FINAL_STATE after $RECOVERIES attempts). PR open; needs human review.","lastUpdate":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","finishedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
DONE_NOT_MERGED
\`\`\`

**Important:** only call \`planflow_task_done\` if the PR actually merged (\`PR_MERGED=true\`).
Otherwise the task stays IN_PROGRESS in PlanFlow so the user can finish it.`
...
1. Inspect the push error output. If it mentions "GH006", "protected",
   ...
   push_with_retry -u origin ${branchName}
   \`\`\`
   b. Open a PR with the exact lowercase title format. Derive the
      default branch first (repos vary between \`main\` and \`master\`):
   \`\`\`bash
   DEFAULT_BRANCH=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's|^origin/||')
   ...
   gh pr create \\
     ...
     --head ${branchName} \\
     --title "feat(<scope>): ${taskId.toLowerCase()} — ${taskName.toLowerCase()}" \\
     --body "Auto-generated by autoExecute. Closes the merge-master request when default branch is protected.
Task: ${taskId}
Summary: <one-line>"
   \`\`\`
...
## 9. Mark task done — ONLY if the work is actually in master

For \`mergeStrategy=auto-merge\`: only run \`planflow_task_done\` when the
poll loop confirmed \`PR_MERGED=true\`. If the PR is still open / queued /
conflicting, leave the task as IN_PROGRESS — the .done marker from step
8.e already records the state, and the user (or a recovery process) can
finish it later. Calling task_done with an unmerged PR creates a lie in
PlanFlow: status says DONE but master doesn't have the code.

For \`mergeStrategy=merge-master\`: master was updated in step 8, always call.
For \`mergeStrategy=pr\` or \`none\`: call as before — the user will merge.
...
`.trim()
  ...
  const mergeLabel =
    mergeStrategy === 'pr'
      ? 'Push & open PR'
      : mergeStrategy === 'auto-merge'
        ? 'Push, open PR & enable GitHub auto-merge (--squash)'
        : mergeStrategy === 'merge-master'
  ...
  const strategyLine =
    mergeStrategy === 'auto-merge'
      ? `auto-merge  (PR opens, GitHub merges when checks pass)`
      : mergeStrategy
  ...
  if (mergeStrategy === 'auto-merge') {
    agentLines.push(
      `note:     repo must have "Allow auto-merge" enabled under Settings → General → Pull Requests`
    )
  }
  agentLines.push(`log:      ${spawnResult.logPath}`)
